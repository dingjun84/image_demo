    ...
    for root, _, files in os.walk(DATA_PATH):
        ...
        for file in files:
            if cur >= start and cur < end :
                imgpaths.append("/data/"+file)
            elif cur >= end :
                break

            cur = cur + 1
        break
    return render_template("list.html",imgpaths=imgpaths,next_start=end,next_end=end+10)
...
@app.route('/searchimg/<image_name>')
def searchimg_path(image_name):
    file_name = os.path.join(app.config['UPLOAD_FOLDER'], image_name)
    if path.exists(file_name):
        return send_file(file_name)
    return "file not exist"
...
def do_search_api():
    ...
    if file:
        ...
        return render_template("index.html", imgpaths=res, searchimg=filename)
